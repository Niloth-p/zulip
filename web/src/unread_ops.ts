...
export function mark_as_unread_from_here(message_id: number): void {
    ...
    const locally_available_matching_message_ids = message_lists.current.all_messages()
    ...
    if (has_found_newest) {
        ...
        const rounded_count =
            Math.floor(locally_available_message_count / UNREAD_COUNT_STEP_SIZE) * UNREAD_COUNT_STEP_SIZE;
